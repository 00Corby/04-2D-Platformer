...
var key = "C220 is the best! It really is, I enjoy this class a lot"
...
func save_game():
	...
	for node in save_nodes:
		if node.filename.empty():
			print("persisent node '%s' is not an instanced scene, skipped")
			continue
		if !node.has_method("save"):
			print("persisten node '%s' is missing a save() function, skipped")
			continue
